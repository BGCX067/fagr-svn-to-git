 ...
 * (when pop is large)
 */
public class CoupledToUniformOutcrossingPopLevelDescendingOrder implements
		OutcrossingStrategy {

	private static final long serialVersionUID = -5803085529540755692L;

	protected static final Logger logger = Logger
			.getLogger(CoupledToUniformOutcrossingPopLevelDescendingOrder.class);
	...
	public CoupledToUniformOutcrossingPopLevelDescendingOrder() {
	...
	public void outcross(List<Population> demes, Population pool) {
		...
		Collections.sort(fao, new OutcrosserDescendingFitnessComparator());
		// // Remember
		// to
		// shuffle
		// fao
		...
		int faoOutcrossings = (int) ((fao.size() > 0 && uniformOutcrossings > 0) ? Math
				.round((double) fao.size() * uniformOutcrossings
						/ (double) uniform.size())
				: (fao.size() > 0 && uniformOutcrossingTendency > 0) ? Binomial
						.staticNextInt(fao.size(), uniformOutcrossingTendency)
						: 0);
		/*
		...
		while (regulatedFaoOutcrossings > 0 && fao.size() > 0) {
