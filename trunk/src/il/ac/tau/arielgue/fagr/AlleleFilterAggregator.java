...
public class AlleleFilterAggregator implements Filter<Outcrosser>,
	...
	private final static Logger logger = Logger.getLogger(AlleleFilterAggregator.class);
	...
	public Number result() {
		if (count){
			...
			if (filtered == 0) {
				logger.debug(getName()+" filtered is ZERO, so result is NaN");
				return Double.NaN;
			} else {
				return aggregated / filtered; // Returns the fraction of alleles
												// that passed the filter out of
												// the total alleles the
												// aggregator goes through
			}
		}
	}
