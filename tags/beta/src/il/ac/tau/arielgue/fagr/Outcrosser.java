...
public class Outcrosser implements Serializable, Comparable<Outcrosser> {
	...
	public double[] getOutcrossingTendency(double density) {
		...
		double reduceIfHeterozygoteModifier = 1;
		if (isHeterozygote())
			reduceIfHeterozygoteModifier = 0.5;
		if (!isFao()) {
			...
			// UniformOutcrossingProbability*1.3333 weighted by (modifier het or hom - this
			// way fao individuals will behave like fao with different
			// strength, instead of a coin toss which will make only half behave
			// like fao and half like uniform. <<although expectancy still might
			// be the same, but then at least variance will be different and
			// variance in fitness is important for the selection>>
			// on the modifier locus) and UniformOutcrossingProbability*0.66668*
			//  weighted by (modifier het or hom, similarly), as a first choice:
			inherentTendency = getUniformOutcrossingProbability()
					* ((1 - reduceIfHeterozygoteModifier) + (reduceIfHeterozygoteModifier
							* getFitness() * -2.0 / 3.0 + reduceIfHeterozygoteModifier * 4.0 / 3.0));
		}
	...
	public boolean isFao() {
		if (fao == null) {
			if (outcrossingModifier == HETEROZYGOT_BA
				...
				fao = true; // changed to this due to getOutcrossingTendency
			} else {
